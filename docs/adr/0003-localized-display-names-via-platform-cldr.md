# ADR 0003: Localized Display Names via Platform CLDR Delegation
...
Bundled, generated per-display-language name tables are **deferred**, tracked as `gap: Bundled Localized Display-Name Tables` in [GAPS.md](../gap/GAPS.md). AI-assisted table maintenance makes that option tractable later if byte-identical cross-platform names or first-class Linux support become requirements.
...
- **Mitigation:** The universal English fallback guarantees a usable, non-null name everywhere; the GAPS entry preserves a concrete path to bundled tables for platforms without native data.
