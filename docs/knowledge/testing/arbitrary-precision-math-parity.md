# Arbitrary-Precision Math Parity

QA · 2026-05-23
Keywords: does our BigDecimal match the JDK, parity scenarios, division by
          zero behaviour, negative remainder, scale-sensitive equality,
          rounding boundary cases, what to test when changing BigInteger

The durable parity protocol for `io.github.aughtone.types.number`. These
scenarios span stories rather than belonging to one: any change to
`BigInteger`, `BigDecimal` or `RoundingMode` is expected to keep every one of
